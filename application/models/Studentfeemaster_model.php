...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function getOnlineFeeCollectionReport($start_date, $end_date)
    {
        ...
        if ($module['is_active']) {
            ...
            $query2 = $this->db->get();
            $result_value2 = $query2->result();

            $result = array_merge($result_value, $result_value2);
        } else {
            $result = $result_value;
        }
        ...
        if (!empty($result)) {
            foreach ($result as $row) {
                // Check if payment_mode property exists before accessing it
                if (isset($row->payment_mode) && $row->payment_mode == "Online") {
                    $array[] = $row;
                }
            }
        }
    ...
    public function getFeeBillingReport($start_date, $end_date) 
    {
        // Get student fees with outstanding balances
        $this->db->select('student_fees_master.id as student_fees_master_id, student_fees_master.amount as master_amount, fee_groups_feetype.amount as fee_amount, fee_groups_feetype.id as fee_groups_feetype_id, students.firstname, students.middlename, students.lastname, students.admission_no, classes.class, sections.section, fee_groups.name as fee_group_name, feetype.type as fee_type, feetype.code as fee_code, fee_groups_feetype.due_date');
        $this->db->from('student_fees_master');
        $this->db->join('student_session', 'student_fees_master.student_session_id = student_session.id');
        ...
        $this->db->join('fee_groups_feetype', 'student_fees_master.id = fee_groups_feetype.fee_session_group_id');
        $this->db->join('fee_groups', 'fee_groups_feetype.fee_groups_id = fee_groups.id');
        $this->db->join('feetype', 'fee_groups_feetype.feetype_id = feetype.id');
        $this->db->where('student_session.session_id', $this->current_session);
        // Filter by date range - only fees with due dates in the selected period
        $this->db->where('fee_groups_feetype.due_date >=', $start_date);
        $this->db->where('fee_groups_feetype.due_date <=', $end_date);
        
        $query = $this->db->get();
        $result = $query->result();
        
        // Calculate outstanding balances for each student fee
        $billing_data = array();
        
        if (!empty($result)) {
            foreach ($result as $row) {
                $amount_paid = 0;
                // Use fee_groups_feetype.amount as the actual fee amount
                $fee_amount = isset($row->fee_amount) ? $row->fee_amount : 0;
                
                // Get ALL payment details for this fee (not just payments in date range)
                ...
                // Check if fee_groups_feetype_id property exists before accessing it
                if (isset($row->fee_groups_feetype_id)) {
                    $this->db->where('fee_groups_feetype_id', $row->fee_groups_feetype_id);
                }
                
                $payment_query = $this->db->get();
                $payments = $payment_query->result();
                
                if (!empty($payments)) {
                    foreach ($payments as $payment) {
                        if (!empty($payment->amount_detail)) {
                            $details = json_decode($payment->amount_detail);
                            if (is_array($details)) {
                                foreach ($details as $detail) {
                                    // Sum ALL payments regardless of date
                                    $amount_paid += isset($detail->amount) ? $detail->amount : 0;
                                }
                            }
                        }
                    }
                ...
                $balance = $fee_amount - $amount_paid;
                
                // Only include records with outstanding balance
                if ($balance > 0) {
                    $billing_entry = new stdClass();
                    $billing_entry->student_name = $this->customlib->getFullName($row->firstname, $row->middlename, $row->lastname, false, false);
                    $billing_entry->admission_no = $row->admission_no;
                    $billing_entry->class = $row->class;
                    $billing_entry->section = $row->section;
                    $billing_entry->fee_group = $row->fee_group_name;
                    $billing_entry->fee_type = $row->fee_type;
                    $billing_entry->amount = $fee_amount;
                    $billing_entry->paid = $amount_paid;
                    $billing_entry->balance = $balance;
                    $billing_entry->due_date = $row->due_date;
                    $billing_data[] = $billing_entry;
                }
            }
        }
